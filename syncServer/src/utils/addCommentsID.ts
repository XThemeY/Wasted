...
(async function () {
  ...
  try {
    ...
    // const peoples = await People.find({ original_name: 'Джонни Депп' });
    1385536;
    for (let i = 11549; i <= 300000; i++) {
      const people = await People.findOne({ id: i });
      await People.findOneAndUpdate(
        { id: i },
        {
          $set: {
            original_name: await translate(people.name, {
              from: 'ru',
              to: 'en',
            }),
          },
